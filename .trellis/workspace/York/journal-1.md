...
## Session 34: AI 数据分析 Prompt 重写

**Date**: 2026-08-02
**Task**: AI 数据分析 Prompt 重写
**Branch**: `main`

### Summary

重写两端共用的那份 AI 分析 Prompt。根因是它从头到尾没告诉模型这些记录是什么行为，却要求判断「频率是否偏高」。本次：明说是自慰记录并硬性约束口吻（中立、不评判、不说教、不建议就医）；人格拆到 system 消息；「数据够不够」的判定权从模型收回代码（不足 10 条或跨度不足 14 天只描述不下结论，达标则必须给方向性判断且禁止敷衍）；新增 RecordSpanDays；删掉写死的 max_tokens（o 系列传旧字段直接 400，此前配推理型模型的用户功能根本不通）且明确不传 temperature；禁止 Markdown 并把数据段自身四十行的短横线前缀改成缩进（模型会模仿输入格式，我们自己的 Prompt 才是泄漏源）；输出格式由两端渲染代码倒推（桌面端按单换行拆、移动端按空行拆，只有段间空行+段内不换行两端都对）；月度趋势补齐连续自然月填 0（此前静默吐掉空月，让「停了三个月」与「连续两月下降」无法区分）；高峰时段只列真实发生过的时段；0 条记录短路不发请求并与本地分析共用文案；本地分析那句「建议适当控制」改为中立措辞。已知未修：补齐后停记用户会看到「趋势稳定，两个月次数相同」而那两个月都是 0，为真但无信息量。测试只能锁住 Prompt 文本里写了这些要求，不保证模型遵守 —— 这是本项目唯一无法静态验证的部分。
